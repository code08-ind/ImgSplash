...
import MainNav, { names } from './MainNav';
...
const Search = () => {
    document.title = names.charAt(0).toUpperCase() + names.slice(1);
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            const url = `https://api.pexels.com/v1/search/?query=${names.toLowerCase()}&page=${number}&per_page=60`;
                ...
                <Grow in>
                    <Grid className={classes.container} container alignItems="stretch" spacing={2}>
                        {images.map((image, index) => (
                            <Grid item xs={12} sm={`${small}`} lg={`${large}`} data-aos="flip-right" style={{ display: 'flex' }}>
                                <a href={image.src[`${view}`]} download target="_blank" rel="noreferrer">
                                    <img className="images" data-aos="flip-right" src={image.src[`${view}`]} key={index} alt="pics" />
                                </a>
                            </Grid>
                        ))}
                    </Grid>
